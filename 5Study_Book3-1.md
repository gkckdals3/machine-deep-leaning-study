{
 "cells": [
  ...
  },
  {
   ...
   "source": [
    "print(knr.score(test_input,test_target))# 테스트 세트의 값이 더 높게 나왔으므로 과소적합"
   ]
  },
  {
   "cell_type": "code",
   ...
   "outputs": [],
   "source": [
    "knr.n_neighbors=3#과대적합:훈련패턴에 너무 집착해서 데이터에 내재된 거시적인 패턴감지가 불가능\n",
    "                 #과소적합:훈련세트와 테스트세트의 성능이 모두 낮거나 테스트 성능이 오히려 높을떄 일어남\n",
    "                 # 이런경우 더 복잡한 모델을 사용해 훈련세트에 잘맞는 모델을 만들어야 한다.(k의값을 낮춘다.)"
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    "print(knr.score(train_input,train_target))\n",
    "print(knr.score(test_input,test_target))# 과소적합도 아니고 과대적합도 아님 "
   ]
  },
  {
   "cell_type": "code",
